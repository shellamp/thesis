        ...
        for url in content['rss']:
            try:
                ...
                for entry in d.entries:
                    ...
                    if now - pub_date > timedelta(days=days): continue
                    link = entry.link
                    if link in articles: continue
                    try:
...
def save_articles(new_articles):
    ...
    all_path = "news_data/all_articles.json"
    ...
    # Save daily file based on today's date
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    with open(f"news_data/raw_articles/{today}.json", 'w') as f:
...
if __name__ == '__main__':
    logging.info("Start scraping script")
    ...
    if not articles:
        ...
        logging.info(f"Scraped {new_count} new articles. Total stored: {total}")
