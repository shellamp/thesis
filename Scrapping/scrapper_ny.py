...
# === CONFIG ===
QUERY = "site:nytimes.com after:2024-12-30 before:2025-04-15"
NUM_RESULTS = 50
OUTPUT_FOLDER = "news_data/nyt_search"
SOURCE_TAG = "nyt"
...
try:
    for url in search(QUERY, num_results=NUM_RESULTS):
        if url.startswith("https://www.nytimes.com/") and url not in article_urls:
...
def extract_date_from_url(url):
    match = re.search(r'nytimes\.com/(\d{4})/(\d{2})/(\d{2})', url)
    if match:
