...
# === CONFIG ===
QUERY = "site:bbc.com/news after:2025-04-11 before:2025-05-5"
NUM_RESULTS = 30
OUTPUT_FOLDER = "news_data/bbc_search"
...
try:
    for url in search(QUERY, num_results=NUM_RESULTS):
        if url.startswith("https://www.bbc.com/news") and url not in article_urls:
            article_urls.append(url)
        time.sleep(1)  # Delay between searches to avoid blocking
...
for month_tag, data in grouped_articles.items():
    raw_file_path = f"{RAW_FOLDER}/{month_tag}.json"
...
# Deduplicate using title + clean_body
unique_articles = {}
seen_keys = set()
